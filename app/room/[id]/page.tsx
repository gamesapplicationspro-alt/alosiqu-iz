...
type Room = { id: string; code: string; phase: GamePhase; questionIndex: number; round: number; version: number; questions: { id: string; text: string; answers: Answer[]; explanation?: string; source?: string }[]; phaseEndsAt: number | null; revealCorrectAnswerId: string | null; paused?: boolean; pausedRemainingMs?: number | null; settings?: { category: string; questionCount: number } };
type Player = { name: string; score: number; participates: boolean; joinedAt: number; answeredRound: number; lastScoredRound: number; lastRoundPoints?: number; lastResponseMs?: number | null };
...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    void refreshState();
    return () => { cancelled = true; if (timer) window.clearTimeout(timer); window.removeEventListener("online", handleOnline); window.removeEventListener("offline", handleOffline); };
  }, [roomId]);
  ...
  async function skipRound() {
    if (!room) return;
    await action(`/api/rooms/${roomId}/skip`, { expectedVersion: room.version });
  }

  async function restartGame() {
    await action(`/api/rooms/${roomId}/restart`);
  }
  ...
  return <main className={`min-h-screen bg-gradient-to-br ${shell}`}>
    <header className={`sticky top-0 z-10 ${headerColor} p-4 text-white shadow-lg`}><div className="mx-auto flex max-w-6xl items-center justify-between gap-4"><div><h1 className="text-lg font-black sm:text-2xl">{isObserver ? "👁️ Λειτουργία παρατήρησης host" : "🏰 Quiz Ιστορίας"}</h1><p className="text-xs opacity-90">Δωμάτιο {room.code} · {phaseTitle[room.phase]}{room.paused ? " · Παύση" : ""}</p></div><div className="flex items-center gap-2"><span aria-live="polite" className={`hidden rounded-full px-3 py-1 text-xs font-bold sm:inline ${connection === "online" ? "bg-green-500/80" : connection === "offline" ? "bg-red-500/80" : "bg-white/20"}`}>{connection === "online" ? "● Online" : connection === "offline" ? "! Εκτός σύνδεσης" : "Σύνδεση…"}</span><div className="rounded-xl bg-white/15 px-3 py-2 text-center"><b className="block text-lg tabular-nums sm:text-2xl">{(me?.score ?? 0).toLocaleString("el-GR")}</b><span className="text-xs">πόντοι</span></div>{room.phaseEndsAt && !room.paused && <div aria-live="off" className="rounded-xl bg-white/15 px-4 py-2 text-center"><b className="text-2xl tabular-nums">{seconds}</b><span className="ml-1 text-sm">δευτ.</span></div>}</div></div></header>
    <section className="mx-auto max-w-5xl p-4 pb-10 sm:p-6">
      ...
      {role === "host" && room.phase !== "lobby" && <div className="mb-4 flex flex-wrap justify-end gap-2"><button disabled={busy || room.phase === "finished"} onClick={() => void skipRound()} className="rounded-xl bg-purple-900 px-4 py-2 font-bold text-white shadow disabled:opacity-50">⏭ Παράλειψη γύρου</button>{(room.phase === "reveal" || room.phase === "scoreboard") && <button disabled={busy} onClick={() => void togglePause()} className="rounded-xl bg-stone-900 px-4 py-2 font-bold text-white shadow disabled:opacity-50">{room.paused ? "▶ Συνέχιση" : "Ⅱ Παύση παιχνιδιού"}</button>}{room.phase === "finished" && <button disabled={busy} onClick={() => void restartGame()} className="rounded-xl bg-amber-700 px-4 py-2 font-bold text-white shadow disabled:opacity-50">↻ Νέο παιχνίδι</button>}</div>}
