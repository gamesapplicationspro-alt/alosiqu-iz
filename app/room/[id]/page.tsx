...
export default function RoomPage() {
  ...
  return <main className={`min-h-screen bg-gradient-to-br ${shell}`}>
    <header className={`sticky top-0 z-10 ${headerColor} p-4 text-white shadow-lg`}><div className="mx-auto flex max-w-6xl items-center justify-between gap-4"><div><h1 className="text-lg font-black sm:text-2xl">{isObserver ? "👁️ Λειτουργία παρατήρησης host" : "🏰 Quiz της Άλωσης"}</h1><p className="text-xs opacity-90">Δωμάτιο {room.code} · {phaseTitle[room.phase]}</p></div><div className="flex items-center gap-2"><div className="rounded-xl bg-white/15 px-3 py-2 text-center"><b className="block text-lg tabular-nums sm:text-2xl">{(me?.score ?? 0).toLocaleString("el-GR")}</b><span className="text-xs">πόντοι</span></div>{room.phaseEndsAt && <div className="rounded-xl bg-white/15 px-4 py-2 text-center"><b className="text-2xl tabular-nums">{seconds}</b><span className="ml-1 text-sm">δευτ.</span></div>}</div></div></header>
