...
export default function SoloQuiz() {
  ...
  useEffect(() => {
    const tick = () => { const currentTime = Date.now(); setNow(currentTime); setSeconds(Math.max(0, Math.ceil((deadline - currentTime) / 1000))); };
  ...
  useEffect(() => {
    ...
    const alreadySaved = history.some((entry) => entry.date === sessionStorage.getItem("soloQuizStartedAt"));
    if (alreadySaved) return;
    history.unshift({ id: crypto.randomUUID(), playerName: name, score, totalQuestions: questions.length, percentage: Math.round((score / (questions.length * 1500)) * 100), date: Date.now(), roomCode: "SOLO", expiresAt: Date.now() + 24 * 60 * 60 * 1000, mode: "solo", results });
  ...
  function restart() { sessionStorage.setItem("soloQuizStartedAt", String(Date.now())); setQuestions(shuffledQuestions()); setIndex(0); setSelected(null); setScore(0); setResults([]); move("question"); }
  ...
  return <main className="min-h-screen bg-gradient-to-br from-amber-50 via-yellow-100 to-orange-100 p-4 sm:p-8"><section className="mx-auto max-w-4xl"><header className="flex items-center justify-between gap-4 rounded-2xl bg-amber-900 p-4 text-white shadow-lg"><div><p className="text-sm opacity-80">Μοναχική πρόκληση</p><h1 className="text-xl font-black">{name}</h1></div><div className="text-right"><b className="text-3xl tabular-nums">{seconds}</b><span> δευτ.</span></div></header><div className="mt-5 flex justify-between font-bold text-amber-900"><span>Ερώτηση {index + 1}/{questions.length}</span><span>{phase === "question" ? "Διάλεξε απάντηση" : phase === "reveal" ? "Αποκάλυψη απάντησης" : resultText}</span></div>
    {phase === "question" && question && <article className="mt-5 rounded-3xl bg-white p-6 shadow-2xl sm:p-10"><h2 className="text-center text-2xl font-black sm:text-3xl">{question.text}</h2><div className="mt-8 grid gap-3 md:grid-cols-2">{question.answers.map((answer, answerIndex) => <button key={answer.id} disabled={Boolean(results.find((result) => result.questionId === question.id))} onClick={() => select(answer.id)} className={`${colors[answerIndex]} min-h-28 rounded-2xl p-5 text-left text-lg font-black text-white shadow-lg transition hover:brightness-110 disabled:opacity-60 ${selected === answer.id ? "ring-4 ring-white ring-offset-4" : ""}`}><span className="mr-3 text-2xl">{["▲", "◆", "●", "■"][answerIndex]}</span>{answer.text}{selected === answer.id && <span className="float-right rounded-lg bg-black/30 px-2 py-1 text-xs">ΠΑΤΗΣΕ ΞΑΝΑ ΓΙΑ ΚΛΕΙΔΩΜΑ</span>}</button>)}</div>{selected && <p className="mt-6 text-center font-bold text-amber-800">Επιλεγμένη απάντηση: πάτησε ξανά για κλείδωμα.</p>}</article>}
